...
const API_URL =
  process.env.LOCAL_IMPORT_API_URL || "http://localhost:3000/api/admin/import";
...
async function main() {
  const itemsJsonPath = fs.existsSync("items.json")
    ? "items.json"
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `Import failed: ${res.status} ${res.statusText} -> ${errText}`,
    );
  }
